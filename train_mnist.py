...
from models import Generator2, Discriminator
...
def main(args):
    ...
    writer = SummaryWriter('./log2/mnist')
    print_str = "\t".join(["{}"] + ["{:.6f}"] * 2)
    ...
    for e in range(epochs):
        ...
        for x, x_label, x_adv in tqdm(train_loader, total=len(train_loader), leave=False):
            ...
            for _ in range(1):
                ...
                y_fake, y_fake_label= D(x_fake)
                # if e >100:
                loss_G = -torch.mean(y_fake.squeeze()) + 0.5*loss_mse(x_fake, x) + 0.5*auxiliary_loss(y_fake_label,x_real_label)
                # else:
