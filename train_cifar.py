...
from models import Generator1, Discriminator
...
def main(args):
    ...
    writer = SummaryWriter('./log2/cifar')
    print_str = "\t".join(["{}"] + ["{:.6f}"] * 2)
    ...
    for e in range(epochs):
        ...
        for x, x_label, x_adv in tqdm(train_loader, total=len(train_loader), leave=False):
            ...
            lambda_gp = 10
            cls = (auxiliary_loss(y_real_label,x_real_label)+auxiliary_loss(y_fake_label,x_real_label))
            ...
            for _ in range(1):
                ...
                y_fake, y_fake_label= D(x_fake)
                # if e >100:
                loss_G = -torch.mean(y_fake.squeeze()) + loss_mse(x_fake, x) + auxiliary_loss(y_fake_label,x_real_label)
                # else:
